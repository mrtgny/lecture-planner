...
const Lecture: FC<ILectureProps> = ({
  ...
  const [{ x, y, transform }, set] = useSpring(() => ({
    x: 0,
    y: 0,
    transform: "scale(1)",
  }));
  ...
  const onDrag: Handlers["onDrag"] = ({ movement: [mx, my] }) => {
    ...
    if (thresholdExceed && !isDragging) {
      ...
      if (isDragging) set({ transform: "scale(1)" });
  ...
  const onDragEnd: Handlers["onDragEnd"] = ({ xy }) => {
    ...
    if (elem && elem.className.includes("lecture-item")) {
      const { width: targetWidth, height: targetHeight } = draggingItem.current
  ...
  return (
    <div className="relative">
      <animated.div
        className="w-[100px] h-[50px] bg-orange-400 rounded-lg flex items-center justify-center text-white cursor-pointer"
      ...
      <Show showIf={!!main && isDragging}>
        <Portal>
          <animated.div
            ref={draggingItem}
            className="top-0 absolute z-10 w-[100px] h-[50px] bg-orange-400 rounded-lg flex items-center justify-center text-white cursor-pointer px-2"
            style={{
              x,
              y,
              transform,
