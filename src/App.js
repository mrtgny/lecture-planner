...
class App extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            days: days ? JSON.parse(days) : Array(10).fill().map(i => Array(6).fill())
        }
        ...
        return (
            <div>
                {courses.map((course, index) => {
                    return (
                        <Course onDragStart={() => this.onDragStart(course)}
                                key={index}
                                course={course}
...
class EditDialog extends React.Component {
    ...
    render() {
        ...
        const dialog = (
            <div>
                <div className='dialog-bg' onClick={onClose}></div>
                <div className='dialog'>
                    <div style={{width: '100%', height: '100%', backgroundColor: 'white', padding: 16}}>
                        <div>
                            <p>Ders Kodu</p>
                            <input type="text" value={this.state.course.code}
                                   style={{width: 'calc(100% - 16px)', padding: 8}}
                                   onChange={e => this.handleOnChange('code', e.target.value)}/>
                        </div>
                        <div>
                            <p>Ders Adı</p>
                            <input type="text" value={this.state.course.name}
                                   style={{width: 'calc(100% - 16px)', padding: 8}}
                                   onChange={e => this.handleOnChange('name', e.target.value)}/>
                        </div>
                        <div>
                            <p>Arkaplan Rengi</p>
                            <input type="color" value={this.state.course.bgColor}
                                   style={{width: 'calc(100% - 16px)', padding: 8}}
                        ...
                        <div style={{width: '100%', display: 'flex', justifyContent: 'space-around'}}>
                            <button style={{padding: 8, margin: 8}}
                                    onClick={onClose}>Vazgeç
                            </button>
                            <button style={{padding: 8, margin: 8}}
...
const Course = props => {
    const {course, onDragStart, onDragEnd, onClick, onClear} = props;
    if (!course)
        return null;
    return (
        <div className='course'
             ...
             onDragEnd={onDragEnd}
             style={{backgroundColor: course.bgColor, color: course.color, position: 'relative'}}>
            <div onClick={onClick} style={{width: '100%', height: '100%'}} className='center'>
                {course.code}
            </div>
            <div onClick={onClear}
                 style={{position: 'absolute', right: 0, top: 0, zIndex: 1, padding: 2}}>x
            </div>
        </div>
    )
};
...
class Schedule extends React.Component {
    ...
    getTime() {
        ...
        const endTime = new Date(new Date().setHours(new Date(this.startTime).getHours() + new Date(this.lectureTime).getHours(), new Date(this.startTime).getMinutes() + new Date(this.lectureTime).getMinutes()));
        this.startTime = new Date().setHours(new Date(this.startTime).getHours() + new Date(this.lectureTime).getHours() + new Date(this.breakTime).getHours(), new Date(this.startTime).getMinutes() + new Date(this.lectureTime).getMinutes() + new Date(this.breakTime).getMinutes());

        const startTimeString = startTime.getHours().toString() + '.' + (startTime.getMinutes().toString() === "0" ? '00' : startTime.getMinutes().toString())
        const endTimeString = endTime.getHours().toString() + '.' + (endTime.getMinutes().toString() === "0" ? '00' : endTime.getMinutes().toString())
    ...
    render() {
        ...
        return days.map((day, dindex) => {
            return <div style={{display: 'flex'}} key={dindex}>
                {
                    day.map((course, cindex) => {
                        return (
                            <div key={cindex}
                                 style={{border: '1px solid #ccc', width: '100%', minHeight: 66}}
                                 className='center'
                            ...
                            >
                                {!dindex && cindex ?
                                    weekDays[cindex - 1] :
                                    !cindex && dindex ? this.times[dindex - 1] :
                                        course ? <Course course={(courses.filter(i => i.id === course) || [])[0]}
